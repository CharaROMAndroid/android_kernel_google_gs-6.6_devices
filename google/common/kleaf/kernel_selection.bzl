...
def _create_prebuilt_target(name, pkg, target):
    ...
    for build, repo in pkg.prebuilt_repos.items():
        ...
        if target == "kernel_aarch64_images":
            native.filegroup(
                name = "{}_{}/{}".format(prefix, build, target),
                srcs = [
            ...
            actual[build] = ":{}_{}/{}".format(prefix, build, target)
    ...
    native.alias(
        ...
        actual = select({
            "@kleaf//build/kernel/kleaf:page_size_16k": actual["kernel_aarch64_16k"],
...
def kernel_selection(name, packages, default = ""):
    ...
    All the targets in KERNEL_TARGETS are aliased to the one in the selected kernel package.
